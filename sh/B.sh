...
mkfifo $file
while true;do 
	wf-recorder   -a 0 -c h264_vaapi  \
		-p preset=veryslow    \
		 \
		-d /dev/dri/renderD128 -f $file <<<y & 
	sleep $wait_time
	while true;do 
		ffmpeg   -readrate 1.0  -f $Type -i $file -vcodec copy -acodec copy -f flv    -hide_banner  \
